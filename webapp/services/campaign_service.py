...
def start_calling_ready_patients(user_id: int) -> dict:
    ...
    ready = db.get_ready_contacts(organization_id)
    if not ready:
    ...
    lists = db.get_contact_lists(organization_id)
    ...
    campaign_id = db.create_campaign(organization_id, data, created_by=user_id)
...
def create_campaign(user_id: int, payload: dict) -> dict:
    ...
    clist = db.get_contact_list(organization_id, int(list_id))
    if not clist:
        raise ValueError("Contact list not found")

    contacts = db.get_valid_contacts_for_list(organization_id, int(list_id))
    if not contacts:
    ...
    campaign = db.get_campaign(organization_id, campaign_id)
...
def list_campaigns(user_id: int) -> list:
    campaigns = db.get_campaigns(db.resolve_organization_id(user_id))
    for c in campaigns:
...
def get_campaign(user_id: int, campaign_id: int) -> dict | None:
    ...
    if campaign.get("current_contact_id"):
        campaign["current_contact"] = db.get_contact(organization_id, campaign["current_contact_id"])
    if campaign.get("current_call_id"):
        campaign["current_call"] = db.get_call(organization_id, campaign["current_call_id"])
...
def start_campaign(user_id: int, campaign_id: int) -> dict:
    ...
    db.add_notification(organization_id, user_id, f"Campaign started: {campaign['name']}", "success")
...
def pause_campaign(user_id: int, campaign_id: int) -> dict:
    ...
    db.add_notification(organization_id, user_id, f"Campaign paused: {campaign['name']}", "info")
    return get_campaign(user_id, campaign_id)
...
def stop_campaign(user_id: int, campaign_id: int) -> dict:
    ...
    db.add_notification(organization_id, user_id, f"Campaign stopped: {campaign['name']}", "info")
    return get_campaign(user_id, campaign_id)
